      ...
      logs{}, rng{std::default_random_engine{}()}
{
    ...
    last_log_y = -Settings::LOG_HEIGHT + dist(rng) + 45;
}
...
void World::update(float dt) noexcept
{
    if (generate_logs)
    {
        ...
        if (logs_spawn_timer >= Settings::TIME_TO_SPAWN_LOGS)
        {
            ...
            float y = std::max(-Settings::LOG_HEIGHT + 43, std::min(last_log_y + dist(rng), Settings::VIRTUAL_HEIGHT + 115 - Settings::LOG_HEIGHT));
